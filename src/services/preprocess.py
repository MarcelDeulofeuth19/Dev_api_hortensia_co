"""Preprocesamiento y features (Regular)."""
import pandas as pd
import numpy as np
import logging
import json
...
from src.config.config import (
    PREPROC_COMMON_ERR,
    PREPROC_COMP_MAP,
    PREPROC_TENDENCIA_COLS_REG,
    PREPROC_NULOS_REGULAR,
    PREPROC_IMPUTAR_REGULAR,
    Var_pct_IPC_3,
    Var_TRM_1,
    IBR_Var,
    Var_TPM_1,
...
departamentos_colombia = cargar_json("data/departamentos_colombia.json")
...
def calcular_variables_cliente(datos_cliente):
    logging.info("[preprocess] calcular_variables_cliente: start | type=%s", type(datos_cliente).__name__)
    # Normalizar entrada a dict
    ...
    # Variables iniciales sobre dict
    ...
    logging.info("[preprocess] calcular_variables_cliente: socio-demográfico OK")
    ...
    logging.info("[preprocess] calcular_variables_cliente: end | cols=%d", len(datos_cliente_df.columns))
...
def preprocesscomportamiento(df):
    ...
    for i in range(1, 25):
        col_comportamiento = f'comportamiento_{i}'
        if col_comportamiento in df.columns:
            df[col_comportamiento] = df[col_comportamiento].astype(str).map(comportamiento_dicc)
        for sector in ["morasMaxSectorFinanciero", "morasMaxSectorReal", "morasMaximas"]:
            col_name = f'{sector}_{i}'
            if col_name in df.columns:
                df[col_name] = df[col_name].astype(str).map(comportamiento_dicc)
    ...
    for i in range(1, 25):
        col_comportamiento = f'comportamiento_{i}'
        if col_comportamiento in df.columns:
            comport_cols.append(col_comportamiento)
    conteos_estados = df[comport_cols].apply(lambda row: row.value_counts(), axis=1).fillna(0).astype(float)
    conteos_estados.columns = [f'estado_{estado}.0' for estado in conteos_estados.columns]
    df = pd.concat([df, conteos_estados], axis=1)
...
def calcular_tendencia(df):
    ...
    columnas = PREPROC_TENDENCIA_COLS_REG
    ...
    cols = ['trimestre_1_saldo', 'trimestre_2_saldo', 'trimestre_3_saldo']
    existing_cols = [col for col in cols if col in df.columns]
    if existing_cols:
        df[existing_cols] = df[existing_cols].fillna(0)

    df["saldos_tri_tendencia"] = df[['trimestre_1_saldo', 'trimestre_2_saldo', 'trimestre_3_saldo']].apply(lambda row: calcular_pendiente(row), axis=1)

    logging.info(f"Tendencia para {col} calculada exitosamente.")

    logging.info(f"Verificando columnas")
    lista_nulos = PREPROC_NULOS_REGULAR
    
    for col in lista_nulos:
        if col not in df.columns:
            df[col] = np.nan  

    lista_imputar = PREPROC_IMPUTAR_REGULAR
